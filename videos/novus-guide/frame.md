...
description: >
  ...
  accent, the three-step gray text ladder, Urbanist (display/numerals/chrome) + Urbanist (body),
...
colors:
  ...
  text-muted: "#5F5C56"
  text-light: "#9A9A9A"
  accent-light: "rgba(227, 95, 0, 0.08)"
  accent-medium: "rgba(227, 95, 0, 0.15)"
  border: "rgba(227, 95, 0, 0.2)"
  card-bg: "rgba(227, 95, 0, 0.04)"
  positive: "#059669"
  negative: "#dc2626"
...
typography:
  # — reading ramp (Urbanist body + Urbanist chrome) —
  body:    { fontFamily: "Urbanist", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "Urbanist", cqw: 0.8, weight: 600, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "Urbanist", px: 12, weight: 500, color: "primary" }
  counter: { fontFamily: "Urbanist", px: 13, weight: 500, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (Urbanist, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "Urbanist", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Urbanist", cqw: 1.9, weight: 700, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "Urbanist", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "Urbanist", cqw: 2.6, weight: 600, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "Urbanist", cqw: 3.0, weight: 700, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "Urbanist", cqw: 4.2, weight: 700, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "Urbanist", cqw: 8.0, weight: 700, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
- **Fonts** — already set to **Urbanist** (display) / **IBM Plex Mono** (body); ignore any preset font name lingering in prose.
...
The voice is two faces in fixed roles: **Urbanist** (display, every numeral, all chrome —
eyebrows uppercase 0.08em) and **Urbanist** (body, muted gray, line 1.6). Headlines are near-black;
...
- **Urbanist** (display/numerals/chrome) + **Urbanist** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (Urbanist body 0.85cqw muted; Urbanist eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (Urbanist `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body Urbanist 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line Urbanist `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + Urbanist label + muted desc + optional
...
blockquote, cite. **Focal** a Urbanist `blockquote` near-black under a 15%-opacity cobalt
...
Urbanist body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a Urbanist `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body Urbanist 400 muted, line 1.6.
...
- **Type** — Urbanist headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; Urbanist body muted line 1.6; ≥1.4cqw floor.
...
- **Urbanist + Urbanist via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.
...
## Novus guide addendum — the paper stage (READ WITH THE FRONTMATTER)

This project is the APP GUIDE: white/warm-paper ground, dark ink, Apple-smooth.

- **Stage**: `#FCFCFA` ground with an optional whisper-soft warm vignette toward
  `#F2F0EC` at the edges. Never a dark frame ground.
- **Screens are the heroes**: captured app screens/clips sit LARGE in soft-shadowed
  cards — flat white card, 14px radius, two-layer shadow
  (`0 2px 4px rgba(35,32,25,0.08), 0 12px 32px rgba(35,32,25,0.10)`), 1px hairline
  `rgba(35,32,25,0.08)`. No glass, no blur — this is the app's own light,
  elevation-not-glass design language.
- **Accent law**: `#E35F00` (the light-ground action orange) at most ONCE per frame,
  only on the thing that asks or the single highlighted control. Solvency green
  `#3DDC97` only as a fill/marker for financial upside (text-grade positive uses
  {colors.positive}); prestige gold `#FFB900` only for the year gate; alert red
  only for damage/filler words.
- **Callouts**: a guide may point — a 2px ink rounded-rect outline or a soft
  orange ring around the UI region the VO names, drawn on (svg-path-draw), one at
  a time, never two simultaneously. Mono chapter kickers: `01 · FOUNDING` style,
  uppercase, tracked, `{colors.text-muted}`.
- **Type**: display/body Urbanist (weights 400–800, sentence case; lowercase for
  display moments), figures/kickers IBM Plex Mono tnum. Ink `#232019` at full
  strength on figures.
- **Motion doctrine unchanged**: VO-cued reveals, power3 long-tail settles, no
  bounce, holds read still; one slow push per chapter at most.
