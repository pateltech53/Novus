...
> **现在只需要一个文件：`supabase/APPLY-ALL.sql`**（0001 → 0018 全部，幂等）。
> 打开 **SQL Editor**，整个文件粘进去 Run 一次即可 —— 全新项目、只跑过一部分
> 迁移、已经跑全了，三种起点都安全：缺什么建什么，已有的不动，粘错项目会在
> 改任何东西之前直接拒绝，最后打印每个迁移一行 `ok`。不记得跑过哪些？先跑
> 只读的 `supabase/CHECK-SCHEMA.sql` 看一眼。下面按文件逐个跑的说明是 0002
> 时代的写法，仍然有效，但已被 APPLY-ALL 取代。
