...
export async function GET(
  ...
  const [user, profile, entitlements, billing, ownedChapters, seat, saves, legacy, board, log] =
    await Promise.all([
      ...
      db.from("entitlements").select("pro, extra_islands, extra_year_closes, industry_packs, cosmetic_bundles, chapter, intent, comp_pro, comp_until, comp_note, rewards_beta").eq("profile_id", id).maybeSingle(),
