...
export async function GET(
  ...
  const [user, profile, entitlements, billing, ownedChapters, seat, saves, legacy, board, log] =
    await Promise.all([
      ...
      db.from("entitlements").select("pro, extra_run_slots, extra_year_closes, industry_packs, cosmetic_bundles, chapter, intent, comp_pro, comp_until, comp_note").eq("profile_id", id).maybeSingle(),
